...
def Jensen_Wake_Model(xHAWT,yHAWT,zHAWT, U_velocity):
    nTurbs = len(xHAWT)
    theta = 0.1
    alpha = sp.tan(theta)
    rho = 1.1716
...
if __name__ == '__main__':
    ...
    for i in range(0, np.size(y)):
        ...
        turbine_y = [turbine_y_bottom, [y[i]], turbine_y_top]
        turbine_x = list(chain.from_iterable(turbine_x))
        turbine_y = list(chain.from_iterable(turbine_y))
        plt.plot(turbine_x, turbine_y, linewidth=2, c='r')
        for j in range(1, np.size(wakes)):
